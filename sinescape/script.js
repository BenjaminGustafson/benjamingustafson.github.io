...
function setup() { "use strict";
  ...
  function draw() {
    ...
    */
    function DrawHills (y, color, r1, r2, r3){
      context.beginPath();
        ...
        for (let i = 0; i <= canvas.width; i++){
            context.lineTo(i, y + Math.sin(i/40+r1)*80 
                                + Math.sin(i/10 + r2)*20 
                                + Math.sin(i/2 + r3)*4);
        }
    ...
    DrawSun(30,
      Math.cos((sliderVal*3.14-.5)*1.5)*-(canvas.width/2-40)+canvas.width/2,
      Math.sin((sliderVal*3.14-.5)*1.5)*-canvas.height/2+canvas.height/2+80,
      "white");
    ...
    r += 40;
    g += 20;
    const start = canvas.height/2+60
    const spacing = 40
    DrawHills(start, `rgb(${r-d},${g-d},${b-d})`,       4.38,1.95,0.61);
    DrawHills(start+spacing, `rgb(${r-d*2},${g-d*2},${b-d*2})`, 2.35,2.41,0.75);
    DrawHills(start+2*spacing, `rgb(${r-d*3},${g-d*3},${b-d*3})`, 0.16,4.10,0.41);
    DrawHills(start+3*spacing, `rgb(${r-d*4},${g-d*4},${b-d*4})`, 1.22,1.15,2.56);
