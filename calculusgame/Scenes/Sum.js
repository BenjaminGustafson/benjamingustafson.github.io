            ...
            switch(sceneNameSplit[2]){
                ...
                case '1':
                    sumSliderLevel(gameState, {numSliders:1, sliderSize:15, gridYMin:-2, gridYMax:2,gridXMin:0,gridXMax:1,
                        magentaTargetValues: [1], yellowTargetValues : [-0.5],
                ...
                case '4':
                    ...
                    break
                case '5':
                    sumMathBlockLevel(gameState, {numSliders:4, sliderSize:15, gridYMin:-2, gridYMax:2,gridXMin:-2,gridXMax:2,
                        magentaTargetValues: [0.5,1,0.5,0], yellowTargetValues : [-0.5,-1,-0.5,-1],
                        greenTracerStart:0, redTracerStart:0,whiteTracerStart:0,
...
function sumMathBlockLevel (gameState, {
    ...
    // Grids
    const gridLeft = new Grid({canvasX:150, canvasY:350, canvasWidth:400, canvasHeight:400, 
        gridXMin:gridXMin, gridYMin:gridYMin, gridXMax:gridXMax, gridYMax:gridYMax, labels:true, arrows:false, autoCellSize: true})
    const gridRight = new Grid({canvasX:500, canvasY:350, canvasWidth:400, canvasHeight:400, 
        gridXMin:gridXMin, gridYMin:gridYMin, gridXMax:gridXMax, gridYMax:gridYMax, labels:true, arrows:false, autoCellSize: true})
    
    // Function tracers
    const greenFunTracer = new FunctionTracer({grid: gridRight})
    ...
    const whiteTracer = new IntegralTracer({grid: gridLeft, input: {}, originGridY:whiteTracerStart, 
...
function sumSliderLevel (gameState, {
    ...
    const redTracer = new IntegralTracer({grid: gridLeft, 
        input:{type:'comboSliders', sliders1:whiteSliders, sliders2:greenSliders, combo: (x,y)=>x+y},
         targets:magentaTargets, originGridY:redTracerStart
    })
    const whiteTracer = new IntegralTracer({grid: gridLeft, input:{type:'sliders', sliders: whiteSliders}, originGridY:whiteTracerStart, 
        unsolvedColor:Color.white,
    })
    const greenTracer = new IntegralTracer({grid: gridLeft, input:{type:'sliders', sliders: greenSliders}, targets:yellowTargets, originGridY:greenTracerStart,
